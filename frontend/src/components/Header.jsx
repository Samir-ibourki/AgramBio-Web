...
function Header() {
  ...
  let Language = ["FR", "EN", "AR"]
  let navLinks = ["Amlou", "Miel", "Argan", "About Us"]
  return (
    <>
      <header className="fixed top-0 left-0 w-full z-[100] transition-all duration-300 bg-black/20 backdrop-blur-md border-b border-white/10">
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
          
          {/* Logo */}
          <div className="text-xl md:text-2xl font-serif font-bold tracking-tight text-cream italic shrink-0">
          ...
          <nav className="hidden md:flex gap-10">
            {navLinks.map((item) => (
              <a key={item} href={`#${item.toLowerCase()}`} className="text-cream/80 hover:text-gold text-xs tracking-widest uppercase transition-all">
                {item}
              </a>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-4 md:gap-6">
            ...
            <div className="relative hidden md:block">
              <button 
                ...
                className="flex items-center gap-1 text-cream hover:text-gold transition-colors cursor-pointer text-xs font-bold"
              >
                <Globe size={18} />
                {lang}
            ...
            {/* shopping cart  */}
            <button className="relative text-cream hover:text-gold transition-colors cursor-pointer p-1">
            ...
            {/* mobile menu  */}
            <button className="md:hidden text-cream p-1 z-[110]" onClick={() => setIsOpen(!isOpen)}>
      ...
      <div className={`fixed inset-0 bg-[#0d0900] z-[90] flex flex-col items-center justify-center transition-all duration-500 ${isOpen ? "opacity-100 visible" : "opacity-0 invisible"}`}>
        ...
        <nav className="flex flex-col items-center gap-8 mb-12">
          {navLinks.map((item) => (
            <a key={item} href={`#${item.toLowerCase()}`} className="text-cream text-3xl font-serif" onClick={() => setIsOpen(false)}>
              {item}
            </a>
          ))}
        </nav>
          ...
          <div className="flex gap-6">
            {Language.map((l) => (
              <button
                ...
                className={`text-lg font-medium transition-all ${lang === l ? 'text-gold' : 'text-cream/40'}`}
              >
                {l}
              </button>
            ))}
